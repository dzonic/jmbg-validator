...
def check_region():
    try:
        ...
        if len(a) < 13:
            ...
            answer.config(font=('arial', 10, 'bold'), text="Region rođenja ili prebivališta: Osijek, Slavonija regija")
            ...
            answer.config(font=('arial', 10, 'bold'), text="Region rođenja ili prebivališta: Sisak, Banovina regija")
            ...
            answer.config(font=('arial', 10, 'bold'), text="Region rođenja ili prebivališta: Ostala mesta u Hrvatskoj")
            ...
            answer.config(font=('arial', 10, 'bold'), text="Region rođenja ili prebivališta: Kragujevac ili Jagodina")
        elif int('%d%d' % (osma_cifra, deveta_cifra)) == 73:
            answer.config(font=('arial', 10, 'bold'), text="Region rođenja ili prebivališta: Niš ili Pirot ili Toplica")
            ...
            answer.config(font=('arial', 10, 'bold'), text="Region rođenja ili prebivališta: Smederevo ili Požarevac")
...
def kontrolna_cifra():
    try:
        ...
        elif len(cif13_pol) == 0:
            answer.config("Nije UNETA CIFRA")
        else:
...
button_validan_jmbg = Button(root, font=('arial', 10, 'bold'), text="Datum", bg="gold",
                             command=provera_datuma)  # .place(x=190,y=200)#.pack()
...
# button_region = Button(root, text="Proveri Region", state="normal",command=check_region) #.pack(side=LEFT, padx=40, pady=30)
# button_region.pack(side=LEFT, padx=40, pady=30)

# Button(root, text="Proveri Region", command=check_region).pack(side=LEFT, padx=40, pady=30)
# Button(root, text="Prikaži pol", command=check_pol).pack()

# def btnPressed(index):
#    for btn in buttons:
#        if str(btn["state"]) == "normal":
#            btn.configure(state = "disabled")
#    buttons[index].configure(state = "active")

# buttons = []
# button_validan_jmbg = Button(root, text = "Validiraj JMBG", command = lambda: [check_number_jmbg(),btnPressed(0)])
# button_validan_jmbg.pack(side=LEFT, padx=50,pady=40)
# buttons.append(button_validan_jmbg)
